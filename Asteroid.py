...
def show_go_screen():
    ...
    try:
        ...
        pickle.dump(HIGH_SCORE, fileobj)
    else:
        HIGH_SCORE = pickle.load(fileobj)
    screen.blit(background, background_rect)
    draw_text(screen, "ASTEROID BLASTER", 64, WIDTH / 2, HEIGHT / 6)
    draw_text(screen, f"HIGH SCORE : {HIGH_SCORE}", 48, WIDTH / 2, HEIGHT / 3)
    draw_text(screen, "Arrow keys move, Space to fire", 32,WIDTH / 2, HEIGHT / 2)
    draw_text(screen, "M and N keys are for volume", 32, WIDTH / 2, HEIGHT * 55/100)
    draw_text(screen, "Press any key to begin", 28, WIDTH / 2, HEIGHT * 3 / 4)
    pygame.display.flip()
    waiting = True
    while waiting:
        clock.tick(FPS)
...
background = pygame.image.load(path.join(img_dir, "preview-1.png")).convert()
...
pygame.mixer.music.load(path.join(snd_dir, 'through_space.ogg'))
